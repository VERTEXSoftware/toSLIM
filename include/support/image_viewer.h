...
void ImageViewer(const std::string& name, unsigned char* data, int w, int h, SLIM_CODE code,bool smoothing = true) {
    ...
    SDL_Window* window = SDL_CreateWindow(
        ...
        std::min(std::max(w,800), 1280), std::min(std::max(h,600), 720),
    ...
    SDL_PixelFormatEnum format = convertSLIMCODEToSDLPixelFormat(code);
    ...
    auto centerImage = [&](int winW, int winH) {
        float scaleX = (float)winW / w;
        float scaleY = (float)winH / h;
        zoom = std::min(scaleX, scaleY);
        offsetX = (winW - w * zoom) / 2.0f;
        offsetY = (winH - h * zoom) / 2.0f;
    };
    ...
    while (running) {
            ...
            switch (ev.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (ev.button.button == SDL_BUTTON_LEFT) {
                        ...
                        clickCount = (ms < 400) ? clickCount + 1 : 1;
                        lastClickTime = now;
        ...
        SDL_Rect dstRect = {(int)(offsetX),(int)(offsetY),(int)(w* zoom),(int)(h * zoom)};
